...
$project = new Project();
$project->status = 1;
$allProjects = $project->getAllProjectsByStatus();
?>
                ...
                <div class="section-heading-wrapper mb-4">
                    ...
                    <div class="filter-search-wrap">
                        <div class="filter-button-wrap" id="courseFilter">
                            ...
                            <button class="filter-btn" value="personal">Personal</button>
                            <button class="filter-btn" value="academic">Academic</button>
                        </div>
                ...
                <div class="row" id="courseContainer">
                    <?php if (!empty($allProjects)) : ?>
                        <?php foreach ($allProjects as $project) : ?>
                            <div class="col-md-6 mb-4" data-category="<?= htmlspecialchars($project['type']); ?>">
                                <div class="course-card-wrapper">
                                    <div class="course-card">
                                        <div class="button-wrapper edit mb-3">
                                            <a href="create-project.php?editProject=<?= htmlspecialchars($project['project_id']); ?>" class="button btn btn-primary mt-2">Edit</a>
                                        </div>
                                        <h5 class="course-title">
                                            <?= htmlspecialchars($project['title']); ?>
                                        </h5>
                                        <p>Type: <?= htmlspecialchars($project['type']); ?></p>
                                        <p>Description: <?= htmlspecialchars($project['description']); ?></p>

                                        <div class="button-wrapper mt-4">
                                            <?php if (!empty($project['github_link'])) : ?>
                                                <a href="<?= htmlspecialchars($project['github_link']); ?>" class="button" target="_blank">Github Link</a>
                                            <?php endif; ?>
                                            <?php if (!empty($project['live_link'])) : ?>
                                                <a href="<?= htmlspecialchars($project['live_link']); ?>" class="button" target="_blank">Live Link</a>
                                            <?php endif; ?>
                                        </div>
                                    </div>
                                </div>
    ...
    <script>
        ...
        function createPagination() {
            ...
            for (let i = 1; i <= totalPages; i++) {
                ...
                // Append the button to the pagination navigation
                paginationNav.appendChild(pageButton);
            }

            // Set the first page as active
            if (paginationNav.firstChild) {
                paginationNav.firstChild.classList.add('active');
            }
        }
