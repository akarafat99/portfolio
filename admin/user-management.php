...
// Handle form submission for enabling or disabling users
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    if (isset($_POST['disableUser'])) {
        $userId = $_POST['disableUser'];
        $user->user_id = $userId;
        $user->status = -2; // Status indicating disable
        $user->updateStatus();
    }
    else if (isset($_POST['enableUser'])) {
        $userId = $_POST['enableUser'];
        $user->user_id = $userId;
        $user->status = 1; // Status indicating enable
        $user->updateStatus();
    }
}
...
$user->status = -2;
$user->user_type = "student";
$allDisabledUsers = $user->getAllUsers();

$allUser = array_merge($allDisabledUsers, $allPendingUsers);
                            ...
                            <div class="mt-3 d-flex justify-content-between">
                                <form method="post" action="">
                                    <?php if ($user['status'] == 1) { ?>
                                        <button type="submit" class="btn btn-danger" name="disableUser" value="<?php echo $user['user_id']; ?>">Disable User</button>
                                    <?php } elseif ($user['status'] == -2) { ?>
                                        <button type="submit" class="btn btn-success" name="enableUser" value="<?php echo $user['user_id']; ?>">Enable User</button>
                                    <?php } ?>
                                </form>
                            </div>
                        </div>
