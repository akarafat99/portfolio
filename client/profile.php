...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['updateImage'])) {
    ...
    $file->status = 1; // Active status
    $file->file_owner = $userId; // Assign owner to the current user
    $file->file_name = "0.jpg"; // Placeholder filename
    $file->file_original_name = "0.jpg"; // Placeholder original filename
    $file_id = $file->insertFile(); // Insert the file record and get the file ID
    ...
    if (isset($_FILES['image'])) {
        ...
        if ($imageFileType == 'jpg' || $imageFileType == 'jpeg' || $imageFileType == 'png') {
            $targetDir = "../store1/"; // Define the target directory for storing files
            $randomString = bin2hex(random_bytes(8)); // Generate a random string
            $fileName = $randomString . $file_id . "." . $imageFileType; // Generate unique file name
            $targetFile = $targetDir . $fileName; // Full path to save the file
            ...
            if (move_uploaded_file($_FILES['image']['tmp_name'], $targetFile)) {
                ...
                $file->setValuesById();
                $file->file_name = $fileName; // Update with the actual file name
                $file->file_original_name = $originalFileName; // Save the original file name
                $file->updateFile();

                // Update the user profile picture ID
                $user = new User();
                $user->user_id = $userId;
                ...
                $session->set("msg1", "Failed to upload file.");
                $session->set("msg1d", 1);
            }
        } else {
            $session->set("msg1", "Invalid file type. Please upload a .jpg, .jpeg, or .png file.");
            $session->set("msg1d", 1);
        }
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['updateProfile'])) {
    $changedUserData = 1;
    $user = new User();
    $user->user_id = $_POST['updateProfile'];
    $user->setAllValues();
    ...
    $user->gender = $gender;
    $user->updateUser(); // Assuming a save method is implemented in the User class

    $session->set("msg1", "Profile updated successfully.");
    $session->set("msg1d", 1);
}

// Get the user data to load the profile
if ($session->exists("user")) {
    if ($changedUserData == 1) {
        ...
        $user1->setAllValues();
        $user = $user1;
        $session->set("user", serialize($user1));
    }
    $serializedUser = $session->get("user");
    $user = unserialize($serializedUser);
                                    ...
                                    <div class="d-inline-block position-relative me-4 mb-3 mb-lg-0 account-profile">
                                        <div class="avatar-preview rounded">
                                            <div id="imagePreview" class="rounded-4 profile-avatar"
                                                style="background-image: url(../store1/<?php echo $file->file_name; ?>);"></div>
                                        </div>
